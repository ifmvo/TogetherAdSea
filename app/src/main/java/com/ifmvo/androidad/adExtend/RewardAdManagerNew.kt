 ...
 */
object RewardAdManagerNew {

    private const val tag = "RewardAdManagerNew"

    private const val adConstStr = TogetherAdConst.reward
    private val configStr by lazy { Config.rewardAdConfig() }

    private val adHelperA by lazy { RewardTempHelper(TogetherAdConst.reward_a) }
    private val adHelperD by lazy { RewardTempHelper(TogetherAdConst.reward_d) }
    ...
    fun requestAd(
        ...
        onClosed: (isReward: Boolean) -> Unit = {}
    ) {
        ...
        adHelperA.requestAd(configStr, object : IAdListener {
            ...
            override fun onAdPrepared(channel: String, adWrapper: AdWrapper) {
                ...
                UmengEvent.eventAdFills(channel, UmengEvent.AD_REWARD_LOCATION)
                timer?.cancel()
                adHelperD.onDestory()
                onSuccess()
            }
